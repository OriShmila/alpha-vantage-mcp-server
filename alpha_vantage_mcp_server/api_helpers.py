...
if not API_KEY:
    raise ValueError("ALPHA_VANTAGE_KEY environment variable required")
...
async def fetch_news_sentiment(
    ...
    :argument: start_time (str): The start time (default: None).
    :argument: end_time (str): The end time (default: None).
    :argument: sort (str): The sort order (default: "LATEST").

    :returns: The news sentiment data. Always returns 10 articles.
    """
    ...
    https_params = {
        ...
        "time_from": start_time,  # Map start_time to time_from for API
        "time_to": end_time,  # Map end_time to time_to for API
        "sort": sort,
        "limit": 10,  # Always return 10 articles
        "apikey": API_KEY,
    }
    return await _make_api_request(https_params, datatype)
...
async def fetch_listing_status(date: str = None, state: str = "active") -> str:
    """
    ...
    :returns: The company listing status data in CSV format.
    """
...
async def fetch_sma(
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Apply simple response limiting for large time series data
        if "Technical Analysis: SMA" in full_response and max_data_points:
            time_series_data = full_response["Technical Analysis: SMA"]
            if len(time_series_data) > max_data_points:
                ...
                limited_data = {
                    ...
                    for date in sorted_dates[:max_data_points]
                }
                full_response["Technical Analysis: SMA"] = limited_data
